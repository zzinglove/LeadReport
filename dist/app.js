...
function metricCard(label, value, note = '', accent = false, delta = null, rate = null, unit = '건') {
  const change = delta === null ? '' : `<div class="kpi-change ${delta >= 0 ? 'up' : 'down'}">${delta >= 0 ? '▲' : '▼'} ${Math.abs(delta)}${unit}${rate === null ? '' : ` · ${Math.abs(rate)}%`}</div>`;
  return `<article class="kpi ${accent ? 'accent' : ''}"><div class="kpi-label">${label}</div><div class="kpi-value">${value}</div>${change}${note ? `<div class="panel-note">${note}</div>` : ''}</article>`;
}
...
function overview() {
  ...
  return `<div class="kpi-grid kpi-grid-five">${metricCard('전체 Lead', fmt(snapshot.total), '현재 스냅샷 기준', false, snapshot.total - snapshot.previous.total, ((snapshot.total - snapshot.previous.total) / snapshot.previous.total * 100).toFixed(1))}${metricCard('Activity', fmt(snapshot.activity), 'Open 제외 전체', false, snapshot.activity - snapshot.previous.activity, ((snapshot.activity - snapshot.previous.activity) / snapshot.previous.activity * 100).toFixed(1))}${metricCard('Qualified', fmt(snapshot.qualified), 'Qualified + Converted', false, snapshot.qualified - snapshot.previous.qualified, ((snapshot.qualified - snapshot.previous.qualified) / snapshot.previous.qualified * 100).toFixed(1))}${metricCard('Converted', fmt(snapshot.converted), '영업기회 전환', true, snapshot.converted - snapshot.previous.converted, ((snapshot.converted - snapshot.previous.converted) / snapshot.previous.converted * 100).toFixed(1))}${metricCard('전환율', `${snapshot.conversionRate}%`, 'Lead → Converted', false, (snapshot.conversionRate - snapshot.previous.conversionRate).toFixed(1), null, '%p')}</div>
  <section class="panel funnel-panel"><div class="panel-head"><div><h3>Lead 구간별 전환율</h3><span class="panel-note">단계별 건수와 전 단계 대비 전환율</span></div><span class="funnel-legend">전주 대비 증감 포함</span></div><div class="funnel">${funnel.map(([name, value, share, transition, delta], index) => `${index ? `<div class="funnel-transition"><span>↓</span><strong>${transition}</strong></div>` : ''}<div class="funnel-stage"><div class="funnel-label"><strong>${name}</strong><span>${fmt(value)}건 · 전체 ${share}</span><small class="stage-change ${delta >= 0 ? 'up' : 'down'}">${delta >= 0 ? '▲' : '▼'} ${Math.abs(delta)}건</small></div><div class="funnel-track"><i style="width:${share.replace('%','')}%"></i></div></div>`).join('')}</div><div class="funnel-footnote">Activity = Open을 제외한 전체 Lead · Qualified = 현재 Qualified + Converted</div></section>
  <div class="section-grid"><section class="panel"><div class="panel-head"><div><h3>Lead 상태 Summary</h3><span class="panel-note">전체 Lead 대비 비중</span></div><span class="funnel-legend">${fmt(statusTotal)}건 = 100%</span></div><div class="status-stack">${status.map(([name, value, color]) => `<i class="${color}" style="width:${value / statusTotal * 100}%" title="${name} ${value / statusTotal * 100}%"></i>`).join('')}</div><div class="status-list">${status.map(([name, value, color]) => `<div class="status-row"><span><i class="status-dot ${color}"></i>${name}</span><div class="bar"><i class="${color}" style="width:${value / statusTotal * 100}%"></i></div><strong>${fmt(value)}<small>${(value / statusTotal * 100).toFixed(1)}%</small></strong></div>`).join('')}</div></section>
