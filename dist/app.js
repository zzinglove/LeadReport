...
function overview() {
  ...
  const funnel = [['Lead', snapshot.total, '100%', ''], ['Activity', snapshot.activity, `${(snapshot.activity / snapshot.total * 100).toFixed(1)}%`, `${(snapshot.activity / snapshot.total * 100).toFixed(1)}%`], ['Qualified', snapshot.qualified, `${(snapshot.qualified / snapshot.total * 100).toFixed(1)}%`, `${(snapshot.qualified / snapshot.activity * 100).toFixed(1)}%`], ['Converted', snapshot.converted, `${(snapshot.converted / snapshot.total * 100).toFixed(1)}%`, `${(snapshot.converted / snapshot.qualified * 100).toFixed(1)}%`]];
  return `<div class="kpi-grid kpi-grid-five">${metricCard('전체 Lead', fmt(snapshot.total), '현재 스냅샷 기준')}${metricCard('Activity', fmt(snapshot.activity), 'Open 제외 전체')}${metricCard('Qualified', fmt(snapshot.qualified), 'Qualified + Converted')}${metricCard('Converted', fmt(snapshot.converted), '영업기회 전환', true)}${metricCard('전환율', `${snapshot.conversionRate}%`, 'Lead → Converted')}</div>
  <section class="panel funnel-panel"><div class="panel-head"><div><h3>Lead 구간별 전환율</h3><span class="panel-note">단계별 건수와 전 단계 대비 전환율</span></div><span class="funnel-legend">전체 대비 비중</span></div><div class="funnel">${funnel.map(([name, value, share, transition], index) => `${index ? `<div class="funnel-transition"><span>↓</span><strong>${transition}</strong></div>` : ''}<div class="funnel-stage"><div class="funnel-label"><strong>${name}</strong><span>${fmt(value)}건 · 전체 ${share}</span></div><div class="funnel-track"><i style="width:${share.replace('%','')}%"></i></div></div>`).join('')}</div><div class="funnel-footnote">Activity = Open을 제외한 전체 Lead · Qualified = 현재 Qualified + Converted</div></section>
