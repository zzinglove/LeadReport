...
function overview() {
  ...
  return `<div class="kpi-grid kpi-grid-five">${metricCard('전체 Lead', fmt(snapshot.total), '현재 스냅샷 기준')}${metricCard('Activity', fmt(snapshot.activity), 'Open + Contacted + Qualified')}${metricCard('Qualified', fmt(snapshot.qualified), 'SQL 전환 검증')}${metricCard('Converted', fmt(snapshot.converted), '영업기회 전환', true)}${metricCard('전환율', `${snapshot.conversionRate}%`, 'Lead → Converted')}</div>
